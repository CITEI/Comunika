...
const ForgotPass: React.VoidFunctionComponent<ForgotPassProps> = (props) => {
  ...
  const [sendButtonAvailable, setSendButtonAvailable] = useState<boolean>(false);
    ...
    <MainContainer>
      <ContentContainer>
        ...
        <VerticalContainer>
          ...
          <Button disabled={!sendButtonAvailable} label="Enviar código" onPress={handleSendCode}></Button>
          <Button disabled={email == ""}label="Já tenho o código" onPress={() => navigation.navigate("ResetPass", {email: email})}/>
        </VerticalContainer>
      </ContentContainer>
    </MainContainer>
  );
